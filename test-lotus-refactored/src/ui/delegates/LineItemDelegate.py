...
class LineItemDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter: QPainter, option, index):
        ...
        if option.state & QStyle.State_Selected:
            painter.fillRect(option.rect, self._theme_service.get_color(ThemeService.SELECTED_BG_COLOR))
        
        # Get text color based on validation status
        text_color = self._theme_service.get_validation_color(validation_status)
