...
class DocumentPresenter(QObject):
    ...
    def __init__(self, document_model: Optional[DocumentModel] = None):
        """
        Initialize the presenter with a document model.
    ...
    def toggle_comment(self, index: int) -> None:
        """
        ...
        self._document_model.toggle_comment(index)
    ...
    def move_line_up(self, index: int) -> None:
        """
        Move a line up one position.
        
        Args:
            index: The index of the line to move
        """
        if not self._document_model:
            return
        
        if self._document_model.move_line_up(index):
            self._update_list_model()
    
    def move_line_down(self, index: int) -> None:
        """
        ...
        if self._document_model.move_line_down(index):
            self._update_list_model()
