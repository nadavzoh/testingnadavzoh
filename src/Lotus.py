...
class Lotus(QMainWindow):
    def __init__(self, fub, spice, af, mutex):
        super().__init__()
        # File paths from command line
        self._fub = fub
        self._spice_file = spice
        self._af_dcfg_file = af
        self._mutex_file = mutex
        # Initialize managers
    ...
    def _setup_mvc(self):
        ...
        # Mutex Dialog - placeholder for now
        # self.mutex_dialog_model = MutexDialogModel()
        self.mutex_dialog_view = BaseDialogView(tab_type="mutex")  # Replace with actual view
        # self.mutex_af_dialog_controller = MutexDialogController()
        ...
        # Mutex Tab
        self._mutex_model = DocumentModel(self._mutex_file, self._spice_file)
        self._mutex_view = DocumentListView()
        self._mutex_controller = DocumentController(
            self._mutex_model,
            self._mutex_view
        )
        self._ui_manager.add_tab(self._mutex_view, "Mutex")
    ...
    def _on_edit_requested(self):
        ...
        # i want to disable only the current tab's buttons..
    ...
    def _on_save_all(self):
        ...
        # self._mutex_controller.save_changes()

    def closeEvent(self, event):
        has_changes = self._af_doc_controller.has_unsaved_changes()
        # or self._mutex_controller.has_unsaved_changes())
