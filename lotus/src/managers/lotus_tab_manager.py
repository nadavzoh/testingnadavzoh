...
from factories.tab_factory import TabFactory
...
class LotusTabManager(QObject):
    ...
    This class manages the application tabs, their controllers, and views.
    It uses TabFactory to create tabs and handles their lifecycle, including
    showing/hiding tabs, handling tab changes, and providing access to
    tab controllers for other parts of the application.

    Attributes:
        ui_manager: Manager for UI components
        files_manager: Manager for accessing file paths
        tabs (list): Container for all tab configurations
        active_tab_index (int): Index of the currently active tab
        tabChanged (pyqtSignal): Signal emitted when the active tab changes
    """
    ...
    def initialize_tabs(self):
        ...
        Creates the initial set of tabs based on the configuration files
        available through the files manager. Sets the first tab as active
        if tabs were created successfully.
        """
        self.add_tab("af",
        ...
        # Set first tab as active if tabs exist, for now - AF is the default one.
        if self.tabs:
    ...
    def add_tab(self, tab_type, config_file, spice_file):
        ...
        Creates a new tab with its associated controllers and views,
        adds them to the UI, and establishes signal connections.
    ...
    def set_active_tab(self, index):
        ...
        Updates the UI to show the corresponding dialog panel
        for the selected tab and updates the filepath in the header.

        Args:
            index (int): Index of the tab to set as active
        """
        if 0 <= index < len(self.tabs):
            self.active_tab_index = index
            tab = self.tabs[index]
            ...
            # Update filepath in the left panel's header
    ...
    def get_active_controller(self):
        ...
        Returns:
            DocumentController: The controller for the currently active tab,
                               or None if there are no tabs.
        """
    ...
    def get_controllers(self):
        ...
        Returns:
            list: List of all document controllers across all tabs.
        """
